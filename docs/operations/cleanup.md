...
```sh
if kubectl api-resources | grep -q idleloomworkloads; then
  test -z "$(kubectl get idleloomworkloads -A -o name)"
  test -z "$(kubectl get idleloomhosts -A -o name)"
  test -z "$(kubectl get idleloomworkloadassignments -A -o name)"
fi
test -z "$(kubectl get nodes -l idleloom-worker=true -o name)"
test -z "$(kubectl -n kube-system get leases \
...
```sh
kubectl delete -k "${IDLELOOM_REPO}/deploy/native/projection" --ignore-not-found
kubectl delete -k "${IDLELOOM_REPO}/deploy/native" --ignore-not-found
...
kubectl get idleloomhosts,idleloomworkloads,idleloomworkloadassignments -A 2>/dev/null
