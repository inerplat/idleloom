...
public endpoint serves the same content. For example, a cluster may reference
`registry.internal.example.com` (private) whose public twin
`mirror.example.com` allows anonymous pulls:

```sh
idlectl create worker evening-worker \
  --kubeconfig "${IDLELOOM_KUBECONFIG}" \
  --context "${IDLELOOM_CONTEXT}" \
  --registry-mirror registry.internal.example.com=https://mirror.example.com
```
